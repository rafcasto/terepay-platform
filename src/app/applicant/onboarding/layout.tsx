...
/** Every editable section used anywhere in the onboarding flow. */
const ONBOARDING_CONTENT_KEYS = [
  'onboarding.layout',
  'borrower.compliance',
...
export default async function OnboardingLayout({ children }: Props) {
  const content = await getContentSections(ONBOARDING_CONTENT_KEYS);
  const layout = content['onboarding.layout'];
