...
/* ---------------------------------------------------------------------------
   ...
   All copy is editable via the content editor (`auth.shared` plus
   `auth.login` / `auth.signup` depending on `mode`).
   --------------------------------------------------------------------------- */
...
function BrandPanel({ mode }: { mode: Mode }) {
  const shared = useSiteContent('auth.shared');
  const page = useSiteContent(mode === 'register' ? 'auth.signup' : 'auth.login');
  return (
    <aside className="relative hidden flex-col overflow-hidden bg-[var(--ink-900)] px-[52px] py-[46px] text-[#EAF0F7] lg:flex">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="mt-1 text-[12px] text-[#8295ab]">{shared.tagline}</div>
        </div>
      </div>
      ...
      <div className="relative z-10 my-auto max-w-[440px] py-12">
        ...
        <h2 className="mt-3.5 font-serif text-[38px] font-semibold leading-[1.12] tracking-tight text-white">
          {page.panelTitleLead} <em className="italic text-[var(--gold-300)]">{page.panelTitleHighlight}</em>.
        </h2>
        <p className="mt-[18px] text-[16px] leading-relaxed text-[#aebccd]">{page.panelBody}</p>
        <ul className="mt-[30px] flex list-none flex-col gap-4 p-0">
          <Tick>{shared.tick1}</Tick>
          <Tick>{shared.tick2}</Tick>
          <Tick>{shared.tick3}</Tick>
        </ul>
      </div>

      <div className="relative z-10 border-t border-white/10 pt-5">
        <p className="text-[12.5px] leading-relaxed text-[#8295ab]">{shared.panelFooter}</p>
...
export function AuthShell({ mode, children }: { mode: Mode; children: ReactNode }) {
  const isReg = mode === 'register';
  const shared = useSiteContent('auth.shared');
  const page = useSiteContent(isReg ? 'auth.signup' : 'auth.login');
  return (
    <div className="grid min-h-screen bg-[var(--surface-page)] lg:grid-cols-[1.05fr_0.95fr]">
      ...
      <div className="flex items-start justify-center px-5 py-10 sm:items-center sm:px-7 sm:py-12">
        <div className="w-full max-w-[424px]">
          ...
          <h1 className="mt-1.5 font-display text-[28px] font-bold tracking-tight text-[var(--text-strong)]">{page.title}</h1>
          <p className="mt-2 text-[15px] leading-normal text-[var(--text-muted)]">{page.subtitle}</p>
          ...
          <div className="mt-6 flex items-start gap-2.5 rounded-xl border border-[var(--orange-200)] bg-[var(--surface-brand-soft)] px-4 py-3.5">
            ...
            <p className="text-[13px] leading-snug text-[var(--orange-900)]">{renderEmphasis(shared.disclosure)}</p>
          </div>
        </div>
      </div>
    </div>
