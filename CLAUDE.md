...
- [docs/AI_CREDIT_ASSESSMENT.md](docs/AI_CREDIT_ASSESSMENT.md) — lender's Results & Decision step → Upstash Redis `assessment:queue` → Pi worker; every input is sent, missing input = 422, result is advisory
...
- **AI credit assessment runs off-site too.** The affordability wizard's Results & Decision step queues `assess_application` jobs on `assessment:queue` (`src/lib/assessment/*`); the same Pi worker serves them ahead of training jobs. The route assembles *every* input the agent needs and refuses with `MISSING_INPUTS` (422) otherwise — never let the agent guess. The result is advisory and is linked to the affordability assessment via `creditAssessmentId`. See [docs/AI_CREDIT_ASSESSMENT.md](docs/AI_CREDIT_ASSESSMENT.md).
- **Model training runs off-site.** `/admin/training` and `/lender/training` (per-user `trainingAccess` grant) only queue jobs / RPCs into Upstash Redis (`training:queue`) and reads status back; the Pi worker in the `credit-assessment-agent` repo does the work and pulls data from `GOOGLE_DRIVE_TRAINING_FOLDER_ID`. Never run parsing/training in a Vercel function. See [docs/MODEL_TRAINING.md](docs/MODEL_TRAINING.md).
...
- **SetPay test cadence (admin, non-production only).** `/admin/setpay-testing` (`systemConfig/setpayTesting`) compresses instalments from fortnightly to N minutes apart (default 4) for consents created while it's on; the minute clock is anchored at disbursement in [src/lib/qippay/schedule-installments.ts](src/lib/qippay/schedule-installments.ts) and instalments carry a `dueAt` ISO datetime sent as `scheduled_for`. The mandate is created as `Daily` with the whole loan as the per-period limit. Hard-locked off when `NEXT_PUBLIC_ENVIRONMENT=production`. "Verify now" on that page runs the same sweep as the payment-refresh cron ([src/lib/qippay/refresh-sweep.ts](src/lib/qippay/refresh-sweep.ts)); the cron bypasses its once-a-day gate while test cadence is on. In stub mode the client reports an instalment collected once its time has passed.
