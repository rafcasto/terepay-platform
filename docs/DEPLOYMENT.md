...
# QIPPAY_MOCK_SETPAY_FAILURE_ENABLED=true      # UAT only — unlocks the "Simulate failure" selector + request override on schedule-payments (hard-off on Vercel production)
# QIPPAY_MOCK_SETPAY_FAILURE_DEFAULT=rejected,error,revoked   # optional — sequence attached to EVERY lodgement (disburse + cron + manual) while set
```
...
- **Simulating a failed collection (UAT only).** Qippay UAT honours a `metadata.mock_setpay_failure` array on `POST /v1/setpay`; each entry plays out on a subsequent day, in order: `rejected` (insufficient funds — consent stays active, retried next day), `error` (bank API outage — retried next day), `revoked` (customer revoked consent in their bank app — **terminal**, a new consent is required). `["rejected","error","revoked"]` = three attempts over three days, then abandoned. Set `QIPPAY_MOCK_SETPAY_FAILURE_ENABLED=true` on the UAT deployment to (a) show a "Simulate failure" selector beside **Schedule pending** on the lender's application page and (b) accept `{ "mockSetpayFailure": [...] }` on `POST /api/applications/<id>/schedule-payments`. **Per test customer:** sign the applicant up with a tagged email — `setpayfail-rejected-error-revoked@example.test`, or a plus-tag on a real inbox so OTP/verification mail still arrives, e.g. `you+setpayfail-rejected@yourdomain.com` — and every instalment lodged for that customer (at disbursement, by the cron, or manually) carries that sequence automatically. Precedence: request body → email tag → `QIPPAY_MOCK_SETPAY_FAILURE_DEFAULT` (a deployment-wide fallback attached to every lodgement while set). The sequence is persisted on the instalment (`scheduledPayments[].mockFailure`) so the panel shows what was simulated. Nothing needs to change at merge/release time: `QIPPAY_MOCK_SETPAY_FAILURE_ENABLED` is simply not set on the production Vercel project, and even if it were, `isSetPayMockFailureEnabled()` returns `false` whenever `VERCEL_ENV=production` — the email tag is never parsed, the selector never renders, and the SetPay client refuses to send the metadata. Qippay currently fires a webhook only for `enduring.status.revoked`; retry/failure webhooks are still to come, so the `rejected`/`error` days will not flip the row to **Retrying** until then — verify them from the Qippay dashboard.
